...
namespace Api.Library.StockMarket
{
    ...
    internal class StockMarketService : LibraryService<StockMarketConfiguration, StockMarketRepository>, IStockMarketService
    {
        ...
        public async Task<List<StockMarketQuoteCandleContract>> GetQuoteCandlesAsync(int quoteId, int timeframeId)
        {
            ...
            switch (timeframe.TimeframeId)
            {
                ...
                case (int)Timeframe.OneYear:
                    {
                        var to = DateTimeOffset.Now.AtTimezone(LibraryExtension.EasternTimezone).Date;
            ...
            while (quoteCandlesResponse.Any() && currentTimeframeMultiplier <= timeframe.Multiplier)
            {
                ...
                var quoteCandleResponseSet = quoteCandlesResponse.Where(quoteCandleResponse => quoteCandleResponse.CreatedOn.Date == quoteCandlesResponse[0].CreatedOn.Date)
                                                                 .ToList();
                var quoteUserAlertsSet = quoteUserAlerts.Where(quoteUserAlert => quoteUserAlert.CreatedOn.AtTimezone(LibraryExtension.EasternTimezone).Date == quoteCandlesResponse[0].CreatedOn.Date)
                                                        .ToList();
